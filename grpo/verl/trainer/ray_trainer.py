...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        if config.algorithm.online_filtering and config.algorithm.online_buffer:
    ...
    def _save_checkpoint(self) -> None:
        ...
        buffer_path = os.path.join(folder_path, "buffer.pkl")
        print(f"Save buffer to disk to resume. path: {buffer_path}")
        if self.config.algorithm.online_filtering and self.config.algorithm.online_buffer and self.batch_buffer is not None:
            self.batch_buffer.save_to_disk(buffer_path)
    ...
    def _load_checkpoint(self) -> None:
        ...
        if os.path.exists(step_file):
            ...
            #     load_step = int(f.readline().strip())
            checkpoint_tracker = json.load(open(step_file, 'r'))
            load_step = checkpoint_tracker['last_global_step']
        ...
        if self.config.algorithm.online_filtering and self.config.algorithm.online_buffer:
            buffer_path = os.path.join(self.config.trainer.load_checkpoint_path, "buffer.pkl")
            if os.path.exists(buffer_path):
                try:
                    print(f"Load buffer from disk to resume. path: {buffer_path}")
                    self.batch_buffer = DataProto.load_from_disk(buffer_path)
                    return
                except:
                    print(f"Load {buffer_path} error. Load an old one.")
                    checkpoint_folder = '/'.join(self.config.trainer.load_checkpoint_path.split('/')[:-1])
                    saves = [f for f in os.listdir(checkpoint_folder) if 'global_step' in f]
                    saves.sort()
                    buffer_path = os.path.join(checkpoint_folder, saves[-2], "buffer.pkl")
                try:
                    print(f"Load buffer from disk to resume. path: {buffer_path}")
                    self.batch_buffer = DataProto.load_from_disk(buffer_path)
                    return
                except:
                    print(f"Load {buffer_path} error. Will start from none.")
                    self.batch_buffer = None
            else:
                print(f"No buffer found at {buffer_path}, will start from none.")
                self.batch_buffer = None
    ...
    def _validate(self) -> Dict[str, Any]:
        ...
        for batch_dict in self.val_dataloader:
            ...
            test_gen_batch = test_batch.pop(
                ...
                non_tensor_batch_keys=["raw_prompt_ids", "multi_modal_data", "mask_idx"],
            )
    ...
    def _make_batch_data(self, metrics: Dict[str, Any]) -> DataProto:
        ...
        while True:
            ...
            new_data_num = len(new_batch) // self.config.worker.rollout.n
            # filter group
            if self.config.algorithm.online_filtering:
                ...
                print('*'*20, 'new_batch after filtering:', len(new_batch), '*'*20)
                new_data_num = len(new_batch) // self.config.worker.rollout.n

                if self.config.algorithm.online_buffer:
                    if self.batch_buffer is None:
                        self.batch_buffer = new_batch[:]
                    else:
                        # keep the buffer size
                        if len(self.batch_buffer) + len(new_batch) > self.buffer_size:
                            self.batch_buffer = self.batch_buffer[(len(self.batch_buffer) + len(new_batch) - self.buffer_size):]
                        self.batch_buffer = DataProto.concat([self.batch_buffer, new_batch[:]])
                    print(f"Add to buffer, Current buffer size: {len(self.batch_buffer)}")
            ...
            if current_batch_size < rollout_batch_size:
                ...
                if self.config.algorithm.online_buffer and self.batch_buffer is not None and len(self.batch_buffer) > rollout_batch_size * self.config.worker.rollout.n:
                    print(f"Buffer has enough data. Sample from buffer, Current buffer size: {len(self.batch_buffer)}")
                    uids = self.batch_buffer.non_tensor_batch["uid"]
                    uids_list = []
                    for uid in uids:
                        if uid not in uids_list:
                            uids_list.append(uid)
                    sample_uids = random.sample(uids_list, rollout_batch_size - current_batch_size)
                    sample_idxs = [idx for idx, uid in enumerate(uids) if uid in sample_uids]
                    sample_batch = self.batch_buffer[sample_idxs]
                    batch = DataProto.concat([batch, sample_batch])
                else:
                    print("Not use buffer or buffer data is relatively small, continue rollout generating.")
            ...
            if current_batch_size < rollout_batch_size:
                ...
                return batch[: self.config.data.rollout_batch_size * self.config.worker.rollout.n], new_data_num
    ...
    def fit(self):
        ...
        while self.global_step < self.training_steps:
            ...
            metrics, timing_raw = {}, {}
            print('*'*20, 'global_step:', self.global_step, '*'*20)
            with timer("step", timing_raw):
                # make a batch of data
                with timer("gen", timing_raw):
                    self.actor_rollout_ref_wg.prepare_rollout_engine()
                    batch, new_data_num = self._make_batch_data(metrics=metrics)
                    self.actor_rollout_ref_wg.release_rollout_engine()
                ...
                if self.config.trainer.critic_warmup <= self.global_step:
                    batch.meta_info["steps"] = [self.global_step, self.training_steps]
                    with timer("update_actor", timing_raw):
                        actor_output = self.actor_rollout_ref_wg.update_actor(batch)
                    print('*'*20, 'update finish.', '*'*20)
            ...
            self.logger.log(data={'new_data_num': new_data_num}, step=self.global_step)
            main_tqdm.update()
