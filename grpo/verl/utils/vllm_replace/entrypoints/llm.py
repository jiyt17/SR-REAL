...
class LLM:
    ...
    def generate(
        ...
        elif "is_embedded" in prompts[0].keys():
            text_tokens = [item["prompt_token_ids"] for item in prompts]
            parsed_prompts = self._convert_v1_inputs(
                prompts=None,
                prompt_token_ids=text_tokens)
            for i, item in enumerate(prompts):
                parsed_prompts[i]["multi_modal_data"] = item["multi_modal_data"]
                parsed_prompts[i]["is_embedded"] = True
        else:
            parsed_prompts = cast(Union[PromptType, Sequence[PromptType]],
                                  prompts)
