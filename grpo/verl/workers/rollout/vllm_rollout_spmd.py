...
def _sample_video_embeds(
    ...
    for b in range(B):
        ...
        print("updated_embed.shape", updated_embed.shape)
...
class vLLMRollout(BaseRollout):
    ...
    def generate_sequences(self, prompts: DataProto) -> DataProto:
        ...
        batch_mask_idx = non_tensor_batch.pop("mask_idx", None)
        ...
        if batch_multi_modal_data is not None:
            ...
            batch_pad_lengths = []
            for raw_prompt_ids, multi_modal_data, mask_idx in zip(batch_raw_prompt_ids, batch_multi_modal_data, batch_mask_idx):
                if self.vila_model:
                    ...
                    multi_modal_data['xyz'] = [_data.to(self.model_vision_encoder.dtype) for _data in multi_modal_data['xyz']]
                    multi_modal_data['mask'] = [
                        _data.to(self.model_vision_encoder.dtype) if _data is not None else None 
                        for _data in multi_modal_data['mask']
                    ]
                    ...
                    media_config = {vision_key: {
                        "frames_split": multi_modal_data[vision_key][0].shape[0] // self.group_frames if "video" in vision_key and self.group_frames>0 else 0,
                        "block_lengths": multi_modal_data['block_lengths']
                    }}
                    inputs_embeds, labels, _ = self.model_vision_encoder._embed(_raw_prompt_ids, multi_modal_data, media_config, labels, None, mask_idx)
                    batch_pad_lengths.append(self.prompt_length - inputs_embeds.shape[1])
                    ...
                    vllm_inputs.append(
                        {"prompt_token_ids": prompt_token_ids, "multi_modal_data": resized_embeds, "is_embedded": True})
                else:
                    vllm_inputs.append(
                        {
                            "prompt_token_ids": list(raw_prompt_ids),
        ...
        with self.update_sampling_params(**prompts.meta_info):
            ...
            print('*'*20, 'inference_engine rollout generation complete', '*'*20)
