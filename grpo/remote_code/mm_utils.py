...
def process_xyzs(xyzs, image_processor, model_cfg, enable_dynamic_res=False, intrinsics=None):
    model_cfg.image_processor = image_processor
    point_maps = [process_xyz(xyz, model_cfg, enable_dynamic_res=enable_dynamic_res) for xyz in xyzs]

    # print("point_maps[0]: ", point_maps[0].shape)
    if all(x.shape == point_maps[0].shape for x in point_maps):
        if len(point_maps[0].shape) == 4:
            point_maps = torch.cat(point_maps, dim=0)
        elif len(point_maps[0].shape) == 3:
            point_maps = torch.stack(point_maps, dim=0)
        else:
            raise ValueError(f"point_maps rank does not equal to 4, rank: {len(point_maps[0].shape)}")
    else:
        raise ValueError("The shape of points in point_maps is different!")
    return point_maps


def dynamic_process_masks(masks, image_hw, data_args, max_tiles=None):
    all_masks = []

    # Loop through masks in each image
    for masks_per_image in masks:
        # NOTE: each masks_per_image is a Mask media (can be 0 length list or multiple in content)
        content = masks_per_image.content
        modality = masks_per_image.modality_type

        if len(content) < 1:
            processed_masks = None
        else:
            processed_masks_per_image = []
            for per_mask in content:
                # this returns a mask in each tiles, concatenated (eg, [7, 448, 448])
                per_mask_in_tiles = process_mask(
                    per_mask, modality, image_hw, data_args, enable_dynamic_res=True, max_tiles=max_tiles
                )
                processed_masks_per_image.append(per_mask_in_tiles)

            # [num_masks, num_tiles, res, res] --> [num_tiles, num_masks, res, res]
            processed_masks = torch.stack(processed_masks_per_image, dim=0)
            processed_masks = rearrange(processed_masks, "n_m n_t ... -> n_t n_m ...")

            if processed_masks is not None:
                # Extend the flat list with each (num_masks, res, res) tensor
                all_masks.extend([processed_masks[tile_idx] for tile_idx in range(processed_masks.shape[0])])

    return all_masks


def process_masks(masks, image_hw, image_processor, model_cfg, enable_dynamic_res=False):
    model_cfg.image_processor = image_processor
    ...
    for masks_per_image in masks:
        ...
        if len(content) < 1:
            ...
            processed_masks_per_image = [
                process_mask(mask, modality, image_hw, model_cfg, enable_dynamic_res=enable_dynamic_res)
                for mask in masks_per_image.content
            ]
            processed_masks = torch.stack(processed_masks_per_image, dim=0)
        masks_list.append(processed_masks)
    # print("processed_masks: ", torch.stack(processed_masks, dim=0).shape)
    return masks_list


def process_mask(
    content, modality_type, image_hw, data_args, enable_dynamic_res=False, enable_dynamic_s2=False, max_tiles=None
):
    ...
    if modality_type == "rle":
        rle = content
        m = cocomask.decode(rle)
        m = m.astype(np.uint8)
    elif modality_type == "segmentation":
        polygen = content
        rle = cocomask.frPyObjects(polygen, image_hw[0], image_hw[1])
        m = cocomask.decode(rle)
        ...
        if image_hw is None:
            assert len(content) == 6, "Expected [x1, y1, x2, y2, h, w] when image_hw is None"
            h, w = content[4], content[5]
        else:
            assert len(content) == 4, "Expected [x1, y1, x2, y2] when image_hw is provided"
            h, w = image_hw

        x1, y1, x2, y2 = clamp([x1, y1, x2, y2], h, w)
    ...
    if "dynamic_s2" in data_args.image_aspect_ratio and enable_dynamic_s2:
        assert crop_size["height"] == crop_size["width"]
        masks, block_size = dynamic_s2_preprocess(
            m, s2_scales=data_args.s2_scales, max_num=data_args.max_tiles, image_size=crop_size["height"]
        )
        masks = [torch.tensor(_mask) for _mask in masks]
        return torch.stack(masks), block_size
    if "dynamic" in data_args.image_aspect_ratio and enable_dynamic_res:
        assert crop_size["height"] == crop_size["width"]
        if max_tiles is not None:
            max_num = max_tiles
            ...
            max_num = data_args.max_tiles
        masks = dynamic_preprocess(m, min_num=data_args.min_tiles, max_num=max_num, image_size=crop_size["height"])
        masks = [torch.tensor(_mask) for _mask in masks]
        return torch.stack(masks)

    mask = cv2.resize(m, (crop_size["width"], crop_size["height"]), interpolation=cv2.INTER_NEAREST)
