...
from .configuration_vila import VILAConfig, ResponseFormat
...
def sort_and_remap_dict(original_dict):
    # 1. 提取所有值并排序
    values = list(original_dict.values())
    # 排序规则：先按第一个元素，再按第二个元素
    ...
    for index, value in enumerate(sorted_values):
        # 注意：将列表转换为元组才能作为字典的键
        value_to_index[tuple(value)] = index
    
    # 3. 构建新的字典
    new_dict = {}
    for key, value in original_dict.items():
        # 原字典的key可能是字符串，转换为整数
        new_dict[key] = value_to_index[tuple(value)]
...
class VILAPretrainedModel(PreTrainedModel):
    ...
    def __init__(self, config: VILAConfig, *args, **kwargs):
        ...
        if len(cfgs) == 3:
            ...
            llm_cfg, vision_tower_cfg, mm_projector_cfg, region_extractor_cfg, perception_encoder_cfg = cfgs
        else:
            raise ValueError(
        ...
        self.perception_encoder = build_perception_encoder(perception_encoder_cfg, config)
        if device_map in ["auto", "cuda"]:
            self.mm_projector = self.mm_projector.cuda()
            self.vision_tower = self.vision_tower.cuda()
            self.region_extractor = self.region_extractor.cuda()
            self.perception_encoder = self.perception_encoder.cuda()
    ...
    def save_pretrained(self, output_dir, state_dict=None, **kwargs):
        ...
        if self.get_perception_encoder():
            print(f"saving perception_encoder to {osp.join(output_dir, 'perception_encoder')}")
            self.perception_encoder.config._name_or_path = osp.join(output_dir, "perception_encoder")
            perception_encoder_state_dict = OrderedDict(
                {k.split("perception_encoder.")[-1]: v for k, v in state_dict.items() if "perception_encoder" in k}
            )
            self.perception_encoder.save_pretrained(
                os.path.join(output_dir, "perception_encoder"),
                state_dict=perception_encoder_state_dict,
            )
            self.config.perception_encoder_cfg = self.perception_encoder.config
    ...
    def post_config(self):
        ...
        # self.region_extractor = self.region_extractor.to(torch.float16)
        # self.perception_encoder = self.perception_encoder.to(torch.float16)
        ######################################################################
        ...
        if getattr(self.config, "region_extractor_cfg", None) is None:
            self.config.region_extractor_cfg = self.region_extractor.config
        if getattr(self.config, "perception_encoder_cfg", None) is None:
            self.config.perception_encoder_cfg = self.perception_encoder.config
    ...
    def freezed_module_patch(self):
        ...
        if self.training:
            ...
            if self.get_perception_encoder() and not getattr(self.config, "tune_perception_encoder", False):
                self.get_perception_encoder().eval()
...
class VILAForCausalLM(VILAPretrainedModel):
    ...
    def encode_images(
        ...
        if getattr(self.config, "dynamic_s2", False):
            raise NotImplementedError("Dynamic S2 for RVILA is not implemented yet.")
        else:
            # original nvila-lite
            if masks is None and xyzs is None:
                assert getattr(self.config, "perception_encoder", None) is None, "perception_encoder is not None"
                image_features = self.get_vision_tower()(images)
                image_features = self.get_mm_projector()(image_features)
                return image_features

            if getattr(self.config, "perception_encoder", None) in ["abs_mlp", "pos_bias_attn"]:
                assert xyzs is not None, "xyzs is required for abs_mlp"
                image_features = self.get_vision_tower()(images)
                image_features_dim = int(image_features.size(1) ** 0.5)

                xyzs = xyzs.permute(0, 3, 1, 2)  # Move channel to second dimension
                lres_xyzs = torch.nn.functional.interpolate(
                    xyzs, size=(image_features_dim, image_features_dim), mode="area"
                )
                lres_flattened_xyzs = rearrange(lres_xyzs.permute(0, 2, 3, 1), "b h w c -> b (h w) c")

                if getattr(self.config, "perception_encoder") == "abs_mlp":
                    lres_xyz_features = self.get_perception_encoder()(lres_flattened_xyzs)
                    # print('xyz added', image_features.shape, lres_xyz_features.shape)
                    image_features = image_features + lres_xyz_features
                elif getattr(self.config, "perception_encoder") == "pos_bias_attn":
                    image_features = self.get_perception_encoder()(lres_flattened_xyzs, image_features)

                # print(f"image_features: {image_features.shape}, lres_xyz_features: {lres_xyz_features.shape}")
                assert len(images) == len(masks), f"images: {len(images)}, masks: {len(masks)}"
                # print(image_features.shape, masks, block_lengths)
                mask_features, dummy_mask_features = self.get_region_extractor()(image_features, masks, block_lengths)
                image_features = self.get_mm_projector()(image_features)
            elif getattr(self.config, "perception_encoder") in ["srgpt_local", "srgpt_global+local"]:
                assert xyzs is not None, "xyzs is required for srgpt_local / srgpt_global+local"
                image_features = self.get_vision_tower()(images)
                image_features_dim = int(image_features.size(1) ** 0.5)
                ...
                assert len(images) == len(masks), f"images: {len(images)}, masks: {len(masks)}"
                mask_features, dummy_mask_features = self.get_region_extractor()(image_features, masks, block_lengths)
                mask_features_pe, dummy_mask_features_pe = self.get_region_extractor()(
                    lres_xyz_features, masks, block_lengths
                )
                image_features = self.get_mm_projector()(image_features)

                if getattr(self.config, "perception_encoder") == "srgpt_global+local":
                    image_features_pe = self.get_mm_projector()(lres_xyz_features)
                    return (
                        image_features,
                        mask_features,
                        dummy_mask_features,
                ...
                image_features = self.get_vision_tower()(images)
                assert len(images) == len(masks), f"images: {len(images)}, masks: {len(masks)}"
                if self.get_region_extractor() is None:
                    mask_features, dummy_mask_features = [], []
                else:
                    mask_features, dummy_mask_features = self.get_region_extractor()(
                        image_features, masks, block_lengths
                    )
                image_features = self.get_mm_projector()(image_features)
        return image_features, mask_features, dummy_mask_features
    ...
    def _embed(
        ...
        media_embeds, dummy_mask_flag = self.__embed_media_tokens(media, media_config, mask_idx)
        ...
        while media_embeds.get("dummy_mask"):
            dummy_mask_embed = media_embeds["dummy_mask"].popleft()
            text_embeds += torch.sum(dummy_mask_embed) * 0

        while media_embeds.get("dummy_image_pe"):
            dummy_image_pe_embed = media_embeds["dummy_image_pe"].popleft()
            text_embeds += torch.sum(dummy_image_pe_embed) * 0

        while media_embeds.get("dummy_mask_pe"):
            dummy_mask_pe_embed = media_embeds["dummy_mask_pe"].popleft()
            text_embeds += torch.sum(dummy_mask_pe_embed) * 0
        ...
        # print(media_tokens) # {151649: 'image', 151650: 'video', 151651: 'mask'}  
        ...
        if dummy_mask_flag:
            assert len(media_embeds['mask']) == 1, "it should have an extra dummy mask"
            mask = media_embeds['mask'].popleft()
            inputs[0] += mask * 0
        ...
        for name in media_embeds:
            if media_embeds[name]:
                if name == "image":
                    ...
                    print(f"Not all {name} embeddings are consumed! But fine. Please check!")
    ...
    def __embed_media_tokens(
        ...
        media_keys = [_key for _key in media.keys() if _key not in ["mask"]]
        ...
        for name in media_keys:
            if self.training:
                ...
                if media.get(name) is None or len(media[name]) == 0:
                    ...
                    if name == "image" and "block_lengths" in media_config[name]:
                        media_config[name]["block_lengths"] = [1]
                    if name == "image":
                        dummy_xyz = dummy.permute(1, 2, 0)
                        media_config[name]["block_lengths"] = [1]
                        # NOTE: SVILA style perception encoder
                        if getattr(self.config, "perception_encoder", None) in ["abs_mlp", "pos_bias_attn"]:
                            dummy_image_features, _, dummy_mask_features = self.encoders[name](
                                [dummy], [dummy_xyz], [None], [None], media_config[name]
                            )
                            embeds["dummy"].extend(dummy_image_features)
                            embeds["dummy_mask"].extend(dummy_mask_features)
                        # NOTE: SRGPT style perception encoder
                        elif getattr(self.config, "perception_encoder", None) in ["srgpt_local", "srgpt_global+local"]:
                            (
                                dummy_image_features,
                                _,
                                dummy_mask_features,
                                dummy_image_features_pe,
                                dummy_mask_features_pe,
                                dummy_mask_features_pe,
                            ) = self.encoders[name].forward_sep(
                                [dummy], [dummy_xyz], [None], [None], media_config[name]
                            )
                            embeds["dummy"].extend(dummy_image_features)
                            embeds["dummy_mask"].extend(dummy_mask_features)
                            embeds["dummy_pe"].extend(dummy_image_features_pe)
                            embeds["dummy_mask_pe"].extend(dummy_mask_features_pe)
                    else:
                        embeds["dummy"].extend(self.encoders[name]([dummy], media_config[name]))
                    continue
            if name == "image":
                assert len(media[name]) == len(
                    media["mask"]
                ), f"media image: {len(media[name])}, media mask: {len(media['mask'])}"
                mask_shape = []
                for m in media['mask']:
                    if m is None:
                        mask_shape.append(None)
                    else:
                        mask_shape.append(m.shape)

                # JYT: dummy mask
                dummy_mask_flag = True
                for mask in media['mask']:
                    if mask is not None:
                        # print(media_config[name]['block_lengths']) 
                        # print(mask.shape) # [region_num, 448, 448]
                        dummy_mask_flag = False
                
                if dummy_mask_flag:
                    sub_img_num = media_config[name]['block_lengths'][0]
                    res = media[name][0].shape[-1]
                    for i in range(sub_img_num):
                        media['mask'][i] = torch.ones((1,res,res), device=media[name][0].device)

                # NOTE: SVILA style perception encoder
                if getattr(self.config, "perception_encoder", None) in ["abs_mlp", "pos_bias_attn"]:
                    if "xyz" in media:
                        image_features, mask_features, dummy_mask_features = self.encoders[name](
                            media[name], media["xyz"], media["mask"], media_config[name]
                        )
                    else:
                        image_features, mask_features, dummy_mask_features = self.encoders[name](
                            media[name], None, media["mask"], media_config[name]
                        )
                    # print('mask_idx', mask_idx)
                    # rearrange mask order
                    if mask_idx is not None:
                        new_mask_features = []
                        ind = 0
                        # for mask_idx_img in mask_idx:
                        mask_idx_img = mask_idx
                        if type(mask_idx_img) is dict:
                            mask_features_img = mask_features[ind: ind + len(mask_idx_img)]
                            new_mask_idx_img = sort_and_remap_dict(mask_idx_img)
                            # print('1', mask_idx_img, new_mask_idx_img, len(mask_features_img))
                            for k,v in new_mask_idx_img.items():
                                new_mask_features.append(mask_features_img[v])
                            ind += len(mask_idx_img)
                        else:
                            mask_features_img = mask_features[ind: ind + mask_idx_img]
                            new_mask_features.extend(mask_features_img)
                            ind += mask_idx_img
                        if ind != 0:
                            mask_features = new_mask_features
                    embeds["image"] = deque(image_features)
                    embeds["mask"] = deque(mask_features)
                ...
                # NOTE: SRGPT style perception encoder
                    ...
                    if "xyz" in media:
                        ...
                        (
                            ...
                            mask_features_pe,
                            dummy_mask_features_pe,
                        ) = self.encoders[name].forward_sep(media[name], None, media["mask"], media_config[name])
                    embeds["image"] = deque(image_features)
                    embeds["mask"] = deque(mask_features)
                    if len(mask_features) == 0:
                        embeds["dummy_mask"].extend(dummy_mask_features)
                    embeds["image_pe"] = deque(image_features_pe)
                    embeds["mask_pe"] = deque(mask_features_pe)
                    if len(mask_features_pe) == 0:
                        embeds["dummy_mask_pe"].extend(dummy_mask_features_pe)
                else:
                    image_features = self.encoders[name].forward_nvila(media[name], media_config[name])
                    embeds["image"] = deque(image_features)
        return embeds, dummy_mask_flag
    ...
    def forward(
        ...
        **kwargs,
        ...
        if inputs_embeds is None:
            inputs_embeds, labels, attention_mask = self._embed(input_ids, media, media_config, labels, attention_mask, mask_idx)
    ...
    def init_depth_model(self):
        ...
        depth_processor = AutoImageProcessor.from_pretrained("path/to/pretrained/Depth-Anything-V2-Large-hf")
        depth_model = AutoModelForDepthEstimation.from_pretrained("path/to/pretrained/Depth-Anything-V2-Large-hf")
    ...
    def generate_content(
        ...
        # to compat with SVILA, check if depth is provided / provide if not
        if getattr(self.config, "perception_encoder", None) is not None and "depth" not in media:
            # if no depth model then init one
            if not hasattr(self, "depth_estimator"):
                self.depth_processor, self.depth_estimator = self.init_depth_model()
                self.depth_estimator.to(self.vision_tower.device)
            ...
            if name == "image":
                if len(media["image"]) == 1 and self.config.image_aspect_ratio in ["dynamic", "dynamic_s2"]:
                    ...
                    if self.config.image_aspect_ratio == "dynamic":
                        images = process_image(media["image"][0], self.config, None, enable_dynamic_res=True).to(dtype)
                        conversation[0]["value"] = conversation[0]["value"].replace(
                            DEFAULT_IMAGE_TOKEN, f"{DEFAULT_IMAGE_TOKEN}\n" * images.shape[0]
                        )
                        block_lengths = [images.shape[0]]
                    else:
                    ...
                    images = process_images(media["image"], self.vision_tower.image_processor, self.config).to(dtype)
                    block_lengths = [1] * images.size(0)
                media[name] = [image for image in images]
                media_config[name]["block_lengths"] = block_lengths
            elif name == "mask":
                if self.config.image_aspect_ratio == "dynamic_s2":
                    raise NotImplementedError("Dynamic S2 is not supported for mask")
                elif self.config.image_aspect_ratio == "dynamic":
                    processed_masks = dynamic_process_masks(media["mask"], None, self.config)
                else:
                    processed_masks = process_masks(media["mask"], None, self.vision_tower.image_processor, self.config)
                media[name] = [_mask.to(dtype).cuda() for _mask in processed_masks]

            elif name == "depth":
                self.config.image_processor = self.vision_tower.image_processor
                xyzs = [process_depth(depth) for depth in media["depth"]]
                if len(media["depth"]) == 1 and self.config.image_aspect_ratio in ["dynamic", "dynamic_s2"]:
                    if self.config.image_aspect_ratio == "dynamic":
                        processed_xyzs = dynamic_process_xyzs(xyzs, self.config)  # processed_xyzs = (x,448,448,3)
                    else:
                        raise NotImplementedError(f"dynamic_s2 not yet supported")
                else:
                    processed_xyzs = process_xyzs(xyzs, self.vision_tower.image_processor, self.config)
                media["xyz"] = [xyz.cuda().to(dtype) for xyz in processed_xyzs]
                media.pop("depth")
                ...
                if self.config.image_aspect_ratio == "dynamic" and self.config.video_max_tiles > 1:
                    ...
                    media[name] = [
                        process_images(images, self.vision_tower.image_processor, self.config).to(dtype)
                        for images in media[name]
                    ]
            else:
                raise ValueError(f"Unsupported media type: {name}")

        # Add placeholder mask for image
        if "mask" not in media and "image" in media:
            media["mask"] = [None] * len(media["image"])
