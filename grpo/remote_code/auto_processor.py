...
from .mm_utils import process_image, process_images, process_depth, dynamic_process_xyzs, process_xyzs, dynamic_process_masks, process_masks
...
class VILAProcessor(ProcessorMixin):
    ...
    def __call__(
        ...
        **kwargs: Unpack[VILAProcessorKwargs],
        ...
        for conv in conversation:
            feat = self.__single_call__(conversation=conv, depth=depth, mask=mask, image_hw=image_hw, **kwargs)
    ...
    def __single_call__(
        ...
        **kwargs: Unpack[VILAProcessorKwargs],
        ...
        if depth:
            media["depth"].append(depth)
        if mask:
            if type(mask) is list:
                media["mask"].extend(mask)
            else:
                media["mask"].append(mask)
        # Process media
        media_config = defaultdict(dict)
        ...
        for name in media_keys:
            if name == "image":
                ...
                self.config.image_processor = self.image_processor
                xyzs = [process_depth(depth) for depth in media["depth"][0]]
                if len(media["depth"][0]) == 1 and self.config.image_aspect_ratio in ["dynamic", "dynamic_s2"]:
                    if self.config.image_aspect_ratio == "dynamic":
                    ...
                    processed_xyzs = process_xyzs(xyzs, self.image_processor, self.config)
                media["xyz"] = processed_xyzs
                media.pop("depth")
                
            elif name == "mask":
                if len(media["mask"]) == 1 and self.config.image_aspect_ratio in ["dynamic"]:
                    processed_masks = dynamic_process_masks(media["mask"], image_hw, self.config)
                else:
                    processed_masks = process_masks(media["mask"], image_hw, self.image_processor, self.config)
                media['mask'] = processed_masks
