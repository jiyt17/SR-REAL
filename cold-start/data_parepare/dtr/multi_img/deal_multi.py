...
def generate_3D_data_spar(
    vg_path: str, pkl_path: str, args=None
):
    ...
    dataset_image_size = {}
    conversations = []
    spar = json.load(open('spar_scannet_multiimg_select_r1.json'))
    print(len(spar))
    res_3D = []
    res_rest = []
    box_num = 0
    ...
    for index in tqdm(range(len(spar))):
        item = spar[index]
        ...
        if 'bbox_img_idx' in item:
            main_frame = item['bbox_img_idx'][0][0]
            box_flag = 1
            bbox_point_img_idx = item['bbox_img_idx'][0]
        elif 'point_img_idx' in item:
            main_frame = item['point_img_idx'][0][0]
            box_flag = 0
            bbox_point_img_idx = item['point_img_idx'][0]
        else:
            res_rest.append(item)
            continue

        # fix main frame to 0
        if 'first' in item['conversations'][0]['value']: # spar question uses first frame as main perspective frame
            main_frame = 0
        frames_extrinsic = {}
        for i in range(len(item['image'])):
            with open('data/SPAR-7M-RGBD/spar/scannet/images/' + item['image'][i].replace('jpg', 'txt').replace('image_color', 'pose')) as f:
            ...
            frames_extrinsic[i] = axis_align_matrix @ cam2global

        # with open('path/to/data/SPAR-7M-RGBD/spar/scannet/images/' + item['image'][main_frame].replace('jpg', 'txt').replace('image_color', 'pose')) as f:
        #     cam2global = [[float(x) for x in line.split()] for line in f.readlines()]
        # cam2global = np.array(cam2global).reshape(4, 4)
        # axis_align_matrix = np.array(scene_data.get("axis_align_matrix"))
        # main_frame_extrinsic = axis_align_matrix @ cam2global

        visual_prompts = []
        for k,v in item.items():
            if box_flag and '_bbox' in k:
                visual_prompts.append(k)
            if not box_flag and '_point' in k:
                visual_prompts.append(k)
        
        visual_prompt_3d = {}
        flag = 1
        for vp_id, img_id in enumerate(bbox_point_img_idx):
            visual_prompt = visual_prompts[vp_id]

            with open('data/SPAR-7M-RGBD/spar/scannet/images/' + item['image'][img_id].replace('jpg', 'txt').replace('image_color', 'pose')) as f:
                cam2global = [[float(x) for x in line.split()] for line in f.readlines()]
            # print(cam2global)

            image_path = os.path.join('data/SPAR-7M-RGBD/spar/scannet/images', item['image'][img_id])
            ...
            if box_flag:
                coord_vp = item[visual_prompt][0]
                obj_num = 0
                for obj_id, obj in obj_3d_2d.items():
                    iou = compute_iou(coord_vp, obj['bbox_2d'])
                    if iou > 0.7:
                        match_obj_id = obj_id
                        match_obj = obj
                        obj_num += 1
                ...
                for obj_id, obj in obj_3d_2d.items():
                    ...
                    dist = math.sqrt((coord_vp[0]-cx)**2 + (coord_vp[1]-cy)**2)
                    if dist < min_dist:
                        match_obj_id = obj_id
                        match_obj = obj
                        obj_num += 1
            ...
            instance = instance_dict[match_obj_id]
            box_3d = instance["bbox_3d"]
            match_obj_i_frame = {}
            for i, frame_extrinsic in frames_extrinsic.items():
                out = project_3d_bbox_to_2d(box_3d, None, frame_extrinsic, intrinsic, image_size)
                if out is not None:
                    bbox_2d, center_cam = out
                    bbox_2d = [int(x * 1000) for x in bbox_2d]
                    match_obj_i_frame[i] = {
            ...
            visual_prompt_3d[visual_prompt] = {
                'current_frame': img_id,
                'current_frame_3d': match_obj,
                'all_frame_3d': match_obj_i_frame
            }
        
        if flag == 0:
            print('skip due to multiple or zero obj match')
            res_rest.append(item)
            continue

        print(visual_prompt_3d)
        # item_region = spar_region[index]
        # item_region['3d_pos'] = visual_prompt_3d
        # res_3D.append(item_region)
        item['3d_pos'] = visual_prompt_3d
        item['main_frame_idx'] = main_frame
        res_3D.append(item)


    print(len(res_3D), len(res_rest))
    # print('box_num:', box_num, box_num_org)
    # print('point_num:', point_num, point_num_org)
    # random.shuffle(res)
    with open('./spar-scannet-multiimg-select-3d-r1.json', 'w') as f:
        json.dump(res_3D, f, indent=4)
    with open('./spar-scannet-multiimg-select-rest.json', 'w') as f:
        json.dump(res_rest, f, indent=4)
...
def main():
    ...
    vg_path = f"/path/to/embodiedscan/embodiedscan_train_vg.json"
    pkl_path = f"/path/to/embodiedscan/embodiedscan_infos_train.pkl"
