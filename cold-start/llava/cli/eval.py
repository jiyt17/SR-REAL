...
def main() -> None:
    ...
    for task, metainfo in TASKS.items():
        ...
        if args.tasks is not None and task not in args.tasks:
            # print(f"Skipping {task} as it is not in {args.tasks}")
            continue
        if args.tags_include is not None and tags.isdisjoint(args.tags_include):
            # print(f"Skipping {task} as it is not in {args.tags_include}")
            continue
        if args.tags_exclude is not None and tags.intersection(args.tags_exclude):
            # print(f"Skipping {task} as it is in {args.tags_exclude}")
            continue
        if "videomme" in task:
            if task.split("-")[-1] not in num_video_frames:
                # print(f"Skipping {task} as it is not in {num_video_frames}")
                continue
        else:
    ...
    for task in tasks:
        ...
        if task.startswith("lmms-"):
            ...
            print(f"task: {task}, _task: {_task}, num_video_frame: {num_video_frame}")
            if args.lora_enable:
                print("Lora Enabled")
                if "_" in task:
                    name, split = task.split("_")
                    cmd += [f"{EVAL_ROOT}/{name}_lora.sh", split]
                else:
                    cmd += [f"{EVAL_ROOT}/{_task}_lora.sh"]
                cmd += [
                    args.lora_path,
                    args.model_path,
                    args.conv_mode,
