        ...
        try:
            ...
            logging.warning(
                f"""one_logger_utils is not installed. Please install it to use one_logger.
                            Please install via `pip install --index-url=<your_private_pypi_index> --upgrade one-logger-utils
`"""
            )
            raise e
    ...
    if training_args.lora_enable:
        ...
        if model.get_vision_tower():
            ...
            if model.get_perception_encoder():
                model.get_perception_encoder().requires_grad_(training_args.tune_perception_encoder)
            mprint(f"vision tower {training_args.tune_vision_tower}")
            mprint(f"mm projector {training_args.tune_mm_projector}")
            mprint(f"region extractor {training_args.tune_region_extractor}")
            if model.get_perception_encoder():
                mprint(f"perception encoder {training_args.tune_perception_encoder}")
        ...
        if not any(
            [
                ...
                training_args.tune_region_extractor,
                training_args.tune_perception_encoder,
            ]
        ):
