...
from llava.model.perception_encoder.builder import build_perception_encoder
...
class LlavaMetaModel(ABC):
    ...
    def init_vlm(self, config: PreTrainedModel = None, *args, **kwargs):
        ...
        if (
            ...
            or hasattr(self, "perception_encoder")
        ):
        ...
        assert (
            ...
            or self.region_extractor is not None
    ...
    def load_pretrained(cls, model_path_or_config, *args, **kwargs):
        ...
        if len(cfgs) == 3:
            ...
            raise ValueError(
                "`llm_cfg` `mm_projector_cfg` `vision_tower_cfg` `region_extractor_cfg` `perception_encoder_cfg` not found in the config."
            )
        ...
        if (
            ...
            or hasattr(vlm, "region_extractor")
            or hasattr(vlm, "perception_encoder")
        ):
            if vlm.is_loaded:
                return vlm
        ...
        vlm.perception_encoder = build_perception_encoder(perception_encoder_cfg, config)
        self.post_config()
        self.is_loaded = True
        ...
        assert (
            ...
            or vlm.perception_encoder is not None
        ), "At least one of the components must be instantiated."
        return vlm
    ...
    def post_config(self):
        ...
        if getattr(self.config, "mm_projector_cfg", None) is None:
            self.config.mm_projector_cfg = self.mm_projector.config
        if getattr(self.config, "region_extractor_cfg", None) is None and self.perception_encoder is not None:
            self.config.region_extractor_cfg = self.region_extractor.config
        if getattr(self.config, "perception_encoder_cfg", None) is None and self.perception_encoder is not None:
            self.config.perception_encoder_cfg = self.perception_encoder.config
    ...
    def encode_images(
        ...
        if getattr(self.config, "dynamic_s2", False):
            ...
            if getattr(self.config, "perception_encoder", None) in ["abs_mlp", "pos_bias_attn"]:
                ...
                image_features = self.get_vision_tower()(images) # [13, 1024, 1152]
                ...
                if getattr(self.config, "perception_encoder") == "abs_mlp":
                    lres_xyz_features = self.get_perception_encoder()(lres_flattened_xyzs) # [13, 1024, 1152]
                    image_features = image_features + lres_xyz_features
                    print('add xyz features')
                ...
                image_features = self.get_mm_projector()(image_features) # [13, 121, 3584]
            else:
...
class LlavaMetaForCausalLM(ABC):
    def _embed(
        ...
        # {151649: 'image', 151650: 'video', 151651: 'mask'}

        # Fuse text and media embeddings
        # combine together into a single sequence, where media embeddings are inserted at the positions of media tokens.
        inputs_m, labels_m = [], []
    ...
    def __embed_media_tokens(
        ...
        for name in media_keys:
            ...
            if name == "image":
                ...
                # print(len(media['image']), len(media["mask"]), media['image'][0].shape, media['mask'][0].shape)
                ...
                # rearrange mask order for multi-view images
                if mask_idx:
                    new_mask_features = []
                    ind = 0
                ...
                embeds["image"] = deque(image_features) # [121+1, 3584]
                embeds["mask"] = deque(mask_features) # region_num [1, 3584]
    ...
    def generate(
        ...
        **generation_kwargs,
        ...
        inputs_embeds, _, attention_mask = self._embed(input_ids, media, media_config, None, attention_mask, mask_idx)
    ...
    def generate_content(
        ...
        media = extract_media(conversation, self.config) # {'image':list}
            ...
            if name == "image":
                if len(media["image"]) == 1 and self.config.image_aspect_ratio in ["dynamic", "dynamic_s2"]:
                    ...
                    if self.config.image_aspect_ratio == "dynamic":
                        images = process_image(media["image"][0], self.config, None, enable_dynamic_res=True).to(dtype) # torch.Size([13, 3, 448, 448])
                        conversation[0]["value"] = conversation[0]["value"].replace(
                            DEFAULT_IMAGE_TOKEN, f"{DEFAULT_IMAGE_TOKEN}\n" * images.shape[0]
                        )
                        block_lengths = [images.shape[0]] # 13 blocks
                    else:
                ...
                # print(block_lengths, images.size())
                media[name] = [image for image in images] # 13
                ...
                elif len(media["mask"]) == 1 and self.config.image_aspect_ratio == "dynamic":
                ...
                media[name] = [
                    _mask.to(dtype).cuda() if _mask is not None else None
                    for _mask in processed_masks 
                ]

            elif name == "depth":
                self.config.image_processor = self.vision_tower.image_processor
                if multiimg_world_xyz is not None:
                    xyzs = multiimg_world_xyz
                else:
                ...
                media["xyz"] = [xyz.cuda().to(dtype) for xyz in processed_xyzs] # 13 blocks
                media.pop("depth")
        ...
        if pre_answer:
            pre_answer_ids = torch.tensor(self.tokenizer.encode(pre_answer)).cuda().unsqueeze(0)
            input_ids = torch.cat([input_ids, pre_answer_ids], 1)
            # print(input_ids)
