...
class VideoProcessor:
    def __init__(
        ...
        self.scene = {}
        print(f"============frame sampling strategy: {self.frame_sampling_strategy}=============")
        ...
        for split in ["train", "val", "test"]:
            with open(os.path.join(self.annotation_dir, f"embodiedscan_infos_{split}.pkl"), "rb") as f:
        ...
        for split in ["train", "val", "test"]:
            ...
            filename = os.path.join(self.video_folder, "metadata", f"scannet_{split}_{box_type}_box.json")
            with open(filename) as f:
                data = json.load(f)
                self.scan2obj.update(data)

        if "mc" in self.frame_sampling_strategy:
            sampling_file = os.path.join(self.video_folder, "metadata", "scannet_select_frames.json")
    ...
    def calculate_world_coords(
        ...
        if dataset_name == "scannet":
            if "depth_cam2img" in meta_info:
                depth_intrinsic = np.array(meta_info.get("depth_cam2img"))[:3, :3]  # Use 3x3 intrinsic matrix
            else:
                depth_intrinsic = np.array(frame.get("depth_cam2img"))[:3, :3]
        else:
            if "cam2img" in meta_info:
                depth_intrinsic = np.array(meta_info.get("cam2img"))[:3, :3]  # Use 3x3 intrinsic matrix
            else:
        ...
        world_coords = unproject(depth_intrinsic.float(), poses.float(), depths.float(), depth_scale)  # (V, H, W, 3)
        ...
        if "cam2img" in meta_info:
            image_intrinsic = np.array(meta_info.get("cam2img"))[:3, :3]  # Use 3x3 intrinsic matrix
        else:
    ...
    def preprocess(
        ...
        flattened_rotated_coords = np.vstack([pm.reshape(-1, 3) for pm in rotated_coords])
        ...
        rgb_frame_files = [os.path.join(self.frame_folder, frame["img_path"]) for frame in frame_files]
