...
def eval_model(args):
    ...
    for instance in tqdm(data_list, total=len(data_list), disable=global_rank != 0):
        ...
        # ques = ques.replace('Choices: ', 'Select the appropriate option from the given choices. ').replace('Please answer directly with only the letter of the correct option and nothing else.', 'Your answer can only include one of options A, B, C or D.')
        if args.reason:
    ...
    if dist.is_main():
        ...
        with open(f"{output_dir}/erqa_output.json", "w") as f:
            json.dump(answer_all, f, indent=2)
        print(f"Saved erqa_output.json to {output_dir}")

        # save accuracy to json
        with open(f"{output_dir}/erqa_accuracy.json", "w") as f:
            json.dump(accuracy_by_question_type, f, indent=2)
        print(f"Saved erqa_accuracy.json to {output_dir}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--question-file",
        type=str,
        default="path/to/SpatialAgent/data/ERQA/erqa.json",
    )
    parser.add_argument(
        "--image-dir", type=str, default="path/to/SpatialAgent/data/ERQA/images"
    )
    parser.add_argument("--generation-config", type=json.loads)
    parser.add_argument(
        "--output-dir",
        type=str,
        default="./svila-spar-cold-verl-hard-erqa",
    )
