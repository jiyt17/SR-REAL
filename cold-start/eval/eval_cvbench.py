...
def eval_model(args):
    ...
    # Read data list
    dataset = load_dataset(args.question_file, split="test")
    print(len(dataset))
    ...
    for instance in tqdm(data_list, total=len(data_list), disable=global_rank != 0):
        ...
        answer_all.append({'question': instance["prompt"], 'answer': instance['answer'], 'response': response, 'output': output, 'question_type': instance['task']})
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--question-file",
        type=str,
        default="path/to/data/CVBench",
    )
    parser.add_argument("--generation-config", type=json.loads)
    parser.add_argument(
        "--output-dir",
        type=str,
        default="./svila-cvbench",
    )
